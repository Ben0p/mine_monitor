...
def groupCallsDay():
    ...
    # 24 hours +8 timezone = 115200
    one_day_ago = time.time() - 115200

    # Generate a list of times for the last 24 hours (per minute)
    # 24 hours = 86400 seconds
    for step in range(0, 86400, 60):
        minute = one_day_ago + step
        minute = datetime.datetime.fromtimestamp(minute + 57600).strftime('%d/%m/%Y %H:%M')
        minutes[minute] = 0

    for call in myresult:
        minute = call[1].timestamp()
